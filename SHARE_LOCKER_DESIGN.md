...
| **共享优先（Share-First）** | 默认查看不销毁，支持多次共享；显式删除才提前失效 | 存一次可给多人看 |
...
| **查看（View）** | 输入凭证 → 返回原始内容（**不销毁**，可多次查看） | `GET /api/view?token=xxx` |
| **删除（Delete）** | 输入凭证 → 提前销毁信息（显式操作，防止误删） | `POST /api/delete` |
...
| POST | `/api/delete` | `{"token": "1234"}` | `{"ok": true}` | 400 格式错 / 404 无效/过期 |
...
| 删除接口 | 无 | 新增 `POST /api/delete` | 显式销毁，防止误删 |
...
# 删除
def delete(token):
    item = lockers.pop(token, None)
    if not item: return 404
    free_pool.append(token)            # 凭证回收到空闲池
    return 200
```
